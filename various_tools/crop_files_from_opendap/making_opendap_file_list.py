...
def main():
    ...
    txt_file_list = open(
        "list_of_global_nc_opendap_files_version_2019_11_beta.txt", "w"
    )
    ...
    for roots, dirs, files in os.walk(local_source_folder, followlinks=True):
        ...
        for file_name in files:
            ...
            # opendap file name
            opendap_filename = local_file_name.replace(
                local_source_folder, opendap_main_folder
            )

            # print only netcdf files and skip cloneMaps directories
            if (
                opendap_filename.endswith(".nc") or opendap_filename.endswith(".nc4")
            ) and ("cloneMaps" not in opendap_filename):
