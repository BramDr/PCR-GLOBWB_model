...
def singleTryNetcdf2PCRobjCloneWithoutTime(
    ...
    if sameClone == False:
        ...
        # crop to cloneMap:
        minX = min(
            abs(f.variables["lon"][:] - (xULClone + 0.5 * cellsizeInput))
        )  # ; print(minX)
        ...
        minY = min(
            abs(f.variables["lat"][:] - (yULClone - 0.5 * cellsizeInput))
        )  # ; print(minY)
...
def singleTryNetcdf2PCRobjClone_version_until_2020_07_14(
    ncFile,
    varName="automatic",
    dateInput=None,
    useDoy=None,
    ...
    if sameClone == False:
        ...
        logger.debug(
            "Crop to the clone map with lower left corner (x,y): "
            + str(xULClone)
            + " , "
            + str(yULClone)
        )
...
def netcdf2PCRobjCloneJOYCE(
    ...
    if sameClone == False:
        ...
        # crop to cloneMap:
        minX = min(
            abs(longitude[:] - (xULClone + 0.5 * cellsizeInput))
        )  # ; print(minX)
        xIdxSta = int(
            np.where(abs(longitude[:] - (xULClone + 0.5 * cellsizeInput)) == minX)[0]
        )
        xIdxEnd = int(math.ceil(xIdxSta + colsClone / (cellsizeInput / cellsizeClone)))
        minY = min(abs(latitude[:] - (yULClone - 0.5 * cellsizeInput)))  # ; print(minY)
        yIdxSta = int(
            np.where(abs(latitude[:] - (yULClone - 0.5 * cellsizeInput)) == minY)[0]
        )
        yIdxEnd = int(math.ceil(yIdxSta + rowsClone / (cellsizeInput / cellsizeClone)))
        cropData = cropData[yIdxSta:yIdxEnd, xIdxSta:xIdxEnd]
...
def netcdf2PCRobjCloneWindDist(
    ...
    if sameClone == False:
        # crop to cloneMap:
        xIdxSta = int(
            np.where(f.variables["lon"][:] == xULClone + 0.5 * cellsizeInput)[0]
        )
        xIdxEnd = int(math.ceil(xIdxSta + colsClone / (cellsizeInput / cellsizeClone)))
        yIdxSta = int(
            np.where(f.variables["lat"][:] == yULClone - 0.5 * cellsizeInput)[0]
        )
        yIdxEnd = int(math.ceil(yIdxSta + rowsClone / (cellsizeInput / cellsizeClone)))
        cropData = f.variables[varName][idx, yIdxSta:yIdxEnd, xIdxSta:xIdxEnd]
    ...
    # convert to PCR object and close f
    outPCR = pcr.numpy2pcr(
        pcr.Scalar, regridData2FinerGrid(factor, cropData, MV), float(0.0)
    )
    f.close()
    f = None
    cropData = None
    ...
    if v == "None":
        ...
        if this_is_a_netcdf_file:
            ...
            if sameClone == True:
                ...
                warp = gdalwarpPCR(
                    v, output, cloneMapFileName, tmpDir, isLddMap, isNomMap
                )
...
def waterAbstractionAndAllocationFAILED(
    water_demand_volume,
    available_water_volume,
    allocation_zones,
    zone_area=None,
    ...
    logger.debug(
        "Allocation of abstraction - then, satisfy demand with neighbour sources."
    )
    ...
    cellAvlWater = pcr.cover(
        pcr.ifthenelse(
            cellAvlWater
            > pcr.areaaverage(
                pcr.ifthen(cellAvlWater > 0.0, cellAvlWater), allocation_zones
            ),
            cellAvlWater,
            0.0,
        ),
    ...
    # allocation water to meet water demand (unit: m3)
    factor = getValDivZero(cellVolDemand, zoneVolDemand, smallNumber)
    factor = pcr.min(0.99, factor)
    factor = pcr.rounddown(factor * 100.0) / 100.0
    addCellAllocation = pcr.min(cellVolDemand, factor * zoneAbstraction)
    addCellAllocation = pcr.ifthenelse(
        addCellAllocation > cellVolDemand,
        cellVolDemand,
        pcr.rounddown(addCellAllocation),
    )
    ...
    # actual water abstraction volume in each cell (unit: m3)
    factor = getValDivZero(cellAvlWater, zoneAvlWater, smallNumber)
    cellAbstraction += factor * zoneAbstraction

    # local abstraction to minimize numerical errors
    additionalLocalAbstraction = pcr.max(
        0.0,
    ...
    additionalLocalAbstraction = pcr.min(
        additionalLocalAbstraction, remainingCellAvlWater
    )
    ...
    zoneDeficitAbstraction = pcr.areatotal(
        cellAllocation, allocation_zones
    ) - pcr.areatotal(cellAbstraction, allocation_zones)
...
def waterAbstractionAndAllocationBeforeRefactoringFinalizing(
    water_demand_volume,
    available_water_volume,
    allocation_zones,
    zone_area=None,
...
def waterAbstractionAndAllocationOLD(
    water_demand_volume,
    available_water_volume,
    allocation_zones,
    zone_area=None,
...
def plot_variable(pcr_variable, filename=None):
    ...
    if filename == None:
        filename = get_random_word(8) + ".map"
