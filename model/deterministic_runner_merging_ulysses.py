...
class DeterministicRunner(DynamicModel):
    ...
    def dynamic(self):
        ...
        if self.modelTime.isLastDayOfMonth():
            ...
            cmd = (
                "python3 "
                + self.configuration.path_of_this_module
                + "/merge_pcraster_maps_6_arcmin_ulysses.py "
                + str(self.modelTime.fulldate)
                + " "
                + str(self.configuration.main_output_directory)
                + "/ states 32 "
    ...
    def merging_netcdf_files(
        ...
        if str(vars(self)[nc_report_type]) != "None":
            ...
            cmd = (
                "python3 "
                + self.configuration.path_of_this_module
                + "/merge_netcdf_6_arcmin_ulysses.py "
                + str(self.configuration.main_output_directory)
                + " "
                + str(self.configuration.main_output_directory)
                + "/global/netcdf/ "
    ...
    def check_pcrglobwb_status(self):
        ...
        if (
            self.configuration.globalOptions["cloneAreas"] == "Global"
            or self.configuration.globalOptions["cloneAreas"] == "part_one"
        ):
        ...
        # for the Ulysses project
            ...
            clone_areas = list(
                set(self.configuration.globalOptions["cloneAreas"].split(","))
            )
