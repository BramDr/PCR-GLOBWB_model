...
class Reporting(object):
    ...
    def additional_post_processing(self):
        ...
        if self._model.routing.floodPlain:
            self.floodDepth = pcr.ifthen(
                self._model.routing.landmask,
                pcr.ifthenelse(
                    pcr.cover(self._model.routing.WaterBodies.waterBodyIds, 0) == 0,
                    self._model.routing.floodDepth,
                    0.0,
                ),
            )
            ...
            self.floodVolume = pcr.ifthen(
                ...
                pcr.ifthenelse(
                    ...
                    pcr.max(
                        0.0,
                        self._model.routing.channelStorage
                        - self._model.routing.channelStorageCapacity,
                    ),
                    0.0,
                ),
            )
        ...
        self.irrPaddyWaterWithdrawal = pcr.ifthen(
            self._model.routing.landmask, self._model.landSurface.irrGrossDemandPaddy
        )
        self.irrNonPaddyWaterWithdrawal = pcr.ifthen(
            self._model.routing.landmask, self._model.landSurface.irrGrossDemandNonPaddy
        )
        self.irrigationWaterWithdrawal = (
            self.irrPaddyWaterWithdrawal + self.irrNonPaddyWaterWithdrawal
        )
        ...
        self.livestockWaterConsumptionVolume = (
            self._model.landSurface.livestockReturnFlowFraction
            * self.livestockWaterWithdrawalVolume
        )
        self.domesticWaterConsumptionVolume = (
            self._model.landSurface.domesticReturnFlowFraction
            * self.domesticWaterWithdrawalVolume
        )
        self.industryWaterConsumptionVolume = (
            self._model.landSurface.industryReturnFlowFraction
            * self.industryWaterWithdrawalVolume
        )
        ##########################################################################################################################################################################################
            ...
            for var in self.outMonthAvgNC:
                ...
                if self._modelTime.endMonth == True:
                    ...
                    vars(self)[var + "MonthAvg"] = (
                        vars(self)[var + "MonthTot"] / self._modelTime.day
                    )
            ...
            for var in self.outAnnuaAvgNC:
                ...
                if self._modelTime.endYear == True:
                    ...
                    vars(self)[var + "AnnuaAvg"] = (
                        vars(self)[var + "AnnuaTot"] / self._modelTime.doy
                    )
    ...
    def ulysses_post_processing(self):
        ...
        theta_sat_upp = self._model.landSurface.soil_topo_parameters[
            "default"
        ].satVolMoistContUpp
        theta_res_upp = self._model.landSurface.soil_topo_parameters[
            "default"
        ].resVolMoistContUpp
        self.ulyssesSMUpp = theta_res_upp + self._model.landSurface.satDegUppTotal * (
            theta_sat_upp - theta_res_upp
        )
        # -- low soil moisture layer
        theta_sat_low = self._model.landSurface.soil_topo_parameters[
            "default"
        ].satVolMoistContLow
        theta_res_low = self._model.landSurface.soil_topo_parameters[
            "default"
        ].resVolMoistContLow
        self.ulyssesSMLow = theta_res_low + self._model.landSurface.satDegLowTotal * (
            theta_sat_low - theta_res_low
        )
        ...
        self.ulyssesSM = (
            self.ulyssesSMUpp * thickness_upp + self.ulyssesSMLow * thickness_low
        ) / (thickness_upp + thickness_low)
