...
class Groundwater(object):
    ...
    def __init__(self, iniItems, landmask, spinUp):
        ...
        if "allocationSegmentsForGroundwater" in list(
            iniItems.groundwaterOptions.keys()
        ):
        ...
        if self.usingAllocSegments:
            ...
            if extrapolate:
                # extrapolate it
                self.allocSegments = pcr.cover(
                    self.allocSegments, pcr.windowmajority(self.allocSegments, 0.5)
                )
            ...
            self.allocSegments = pcr.clump(self.allocSegments)
            cell_ids = (
                pcr.mapmaximum(pcr.scalar(self.allocSegments))
                + pcr.scalar(100.0)
