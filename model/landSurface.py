...
class LandSurface(object):
    ...
    def waterDemandOptions(self, iniItems):
        ...
        if self.domesticWaterDemandOption:
            self.domesticWaterDemandFile = vos.getFullPath(
                iniItems.landSurfaceOptions["domesticWaterDemandFile"],
                self.inputDir,
                False,
            )
        ...
        if self.industryWaterDemandOption:
            self.industryWaterDemandFile = vos.getFullPath(
                iniItems.landSurfaceOptions["industryWaterDemandFile"],
                self.inputDir,
                False,
            )
        ...
        if self.livestockWaterDemandOption:
            self.livestockWaterDemandFile = vos.getFullPath(
                iniItems.landSurfaceOptions["livestockWaterDemandFile"],
                self.inputDir,
                False,
            )
        ...
        if iniItems.landSurfaceOptions["desalinationWater"] not in ["None", "False"]:
            ...
            self.desalinationWaterFile = vos.getFullPath(
                iniItems.landSurfaceOptions["desalinationWater"], self.inputDir
            )
        else:
    ...
    def partitioningGroundSurfaceAbstraction(self, groundwater, routing):
        ...
        swAbstractionFractionDict["treshold_to_maximize_irrigation_surface_water"] = (
    ...
    def update(self, meteo, groundwater, routing, currTimeStep):
        ...
        if self.includeIrrigation and (
            currTimeStep.doy == 1 or currTimeStep.timeStepPCR == 1
        ):
            logger.info("Setting irrigation water efficiency")
            for coverType in self.coverTypes:
                self.landCoverObj[coverType].updateIrrigationWaterEfficiency(
                    currTimeStep
                )
