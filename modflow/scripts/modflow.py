...
class ModflowCoupling(object):
    ...
    def __init__(self, configuration, currTimeStep):
        ...
        # read the ldd map
        self.lddMap = vos.netcdf2PCRobjCloneWithoutTime(
            configuration.modflowParameterOptions["channelNC"],
            "lddMap",
    ...
    def createSubmodels(self):
        ...
        self.modflow = groundwater_MODFLOW.GroundwaterModflow(
            self.configuration, self.landmask
        )
    ...
    def dumpState(self, outputDirectory, timeStamp="Default"):
        ...
        for variable, map in groundWaterState.iteritems():
            vos.writePCRmapToDir(
                ...
                str(variable) + "_" + timeStamp + ".masked.map",
                outputDirectory,
            )
    ...
    def dumpVariableValuesForPCRGLOBWB(self, outputDirectory, timeStamp="Default"):
        ...
        if (
            self._configuration.online_coupling_between_pcrglobwb_and_modflow
            and self._configuration.steady_state_only == False
        ):
            filename = (
                outputDirectory
                + "/modflow_files_for_"
    ...
    def update(self):
        ...
        if self._modelTime.isLastDayOfMonth() or self._modelTime.isLastTimeStep():
            ...
            logger.info(
                "Save or dump states to pcraster maps for time %s to the directory %s",
                self._modelTime,
            ...
            logger.info(
                "Save/dump some variables for PCR-GLOBWB simulation to pcraster maps to the directory %s",
                self._configuration.endStateDir,
            )
    ...
    def get_initial_heads(self):
        logger.info(
            "Get initial head values (based on a steady-state simulation or a pre-defined pcraster map."
        )
        ...
        logger.info(
            "Save/dump states of the initial conitions used to pcraster maps to the directory %s",
            self._configuration.endStateDir,
        )
        self.dumpState(
            outputDirectory=self._configuration.endStateDir,
            timeStamp=self._configuration.globalOptions["startTime"] + ".ini",
        )

        # save/dump groundwater depth
        logger.info(
            "Save/dump groundwater depth maps to the directory %s",
            self._configuration.endStateDir,
        )
        self.dumpGroundwaterDepth(
            outputDirectory=self._configuration.endStateDir,
            timeStamp=self._configuration.globalOptions["startTime"] + ".ini",
        )
        ...
        if (
            self._configuration.steady_state_only
            or self._configuration.modflowTransientInputOptions[
                "usingPredefinedInitialHead"
            ]
            == "False"
        ):
            logger.info(
            ...
            self.dumpVariableValuesForPCRGLOBWB(
                outputDirectory=self._configuration.mapsDir,
                timeStamp=self._configuration.globalOptions["startTime"] + ".ini",
            )
