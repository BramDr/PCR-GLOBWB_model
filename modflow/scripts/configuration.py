...
class Configuration(object):
    ...
    def __init__(
        self, iniFileName, debug_mode=False, no_modification=True, system_arguments=None
    ):
    ...
    def repair_ini_key_names(self):
        ...
        if (
            "timeStep" in self.globalOptions.keys()
        ...
        if (
            "irrigationEfficiency" not in self.landSurfaceOptions.keys()
            or self.landSurfaceOptions["irrigationEfficiency"] == "False"
            or self.landSurfaceOptions["irrigationEfficiency"] == "None"
        ):
            logger.warning(
                'The "irrigationEfficiency" map is not defined in the configuration file. This run assumes 100% efficiency.'
            )
        ...
        if "pumpingCapacityNC" not in self.groundwaterOptions.keys():
            ...
            msg += (
                'is not defined in the "groundwaterOptions" of the configuration file. '
            )
        ...
        if (
            ...
            not in self.landSurfaceOptions.keys()
        ):
        ...
        if "m2tChannelDischargeLongIni" in self.routingOptions.keys():
            self.routingOptions["m2tDischargeLongIni"] = self.routingOptions[
                "m2tChannelDischargeLongIni"
            ]
        #
        if "waterBodyStorageIni" not in self.routingOptions.keys():
            logger.warning(
                "Note that 'waterBodyStorageIni' is not defined in the ini/configuration file will be calculated from 'channelStorageIni'."
            )
        ...
        if "avgChannelDischargeIni" in self.routingOptions.keys():
            self.routingOptions["avgDischargeLongIni"] = self.routingOptions[
                "avgChannelDischargeIni"
            ]
        #
        if "m2tChannelDischargeIni" in self.routingOptions.keys():
            self.routingOptions["m2tDischargeLongIni"] = self.routingOptions[
                "m2tChannelDischargeIni"
            ]
        #
        if "avgBaseflowIni" in self.routingOptions.keys():
            self.routingOptions["avgBaseflowLongIni"] = self.routingOptions[
                "avgBaseflowIni"
            ]

        if "avgInflowLakeReservIni" in self.routingOptions.keys():
            self.routingOptions["avgLakeReservoirInflowShortIni"] = self.routingOptions[
                "avgInflowLakeReservIni"
            ]

        if "avgOutflowDischargeIni" in self.routingOptions.keys():
            self.routingOptions["avgLakeReservoirOutflowLongIni"] = self.routingOptions[
                "avgOutflowDischargeIni"
            ]

        if "avgDischargeShortIni" not in self.routingOptions.keys():
            logger.warning(
                'The initial condition "avgDischargeShortIni" is not defined. "avgDischargeLongIni" is used in this run.'
            )
            self.routingOptions["avgDischargeShortIni"] = self.routingOptions[
                "avgDischargeLongIni"
            ]

        if "avgSurfaceWaterInputLongIni" in self.routingOptions.keys():
            logger.warning(
                "Note that avgSurfaceWaterInputLongIni is not used and not needed in the ini/configuration file."
            )
        ...
        if "subDischargeIni" not in self.routingOptions.keys():
            ...
            logger.warning(msg)
            self.routingOptions["subDischargeIni"] = self.routingOptions[
        ...
        if (
            ...
            not in self.groundwaterOptions.keys()
        ):
        ...
        if (
            ...
            logger.warning(msg)
            self.groundwaterOptions["avgNonFossilGroundwaterAllocationShortIni"] = (
                self.groundwaterOptions["avgNonFossilGroundwaterAllocationLongIni"]
            )
        ...
        if (
            ...
            logger.warning(msg)
            self.landSurfaceOptions["treshold_to_maximize_irrigation_surface_water"] = (
                "0.0"
            )
        ...
        if (
            ...
            logger.warning(msg)
            self.groundwaterOptions["maximumDailyFossilGroundwaterAbstraction"] = (
                "0.020"
            )
        ...
        if (self.landSurfaceOptions["includeDomesticWaterDemand"] == "False") and (
            "domesticWaterDemandFile" not in self.landSurfaceOptions.keys()
        ):
        ...
        if (self.landSurfaceOptions["includeIndustryWaterDemand"] == "False") and (
            "industryWaterDemandFile" not in self.landSurfaceOptions.keys()
        ):
        ...
        if (self.landSurfaceOptions["includeLivestockWaterDemand"] == "False") and (
            "livestockWaterDemandFile" not in self.landSurfaceOptions.keys()
        ):
        ...
        if "precipitation_set_per_year" not in self.meteoOptions.keys():
            self.meteoOptions["precipitation_set_per_year"] = "False"
        if "temperature_set_per_year" not in self.meteoOptions.keys():
            self.meteoOptions["temperature_set_per_year"] = "False"
        if "refETPotFileNC_set_per_year" not in self.meteoOptions.keys():
