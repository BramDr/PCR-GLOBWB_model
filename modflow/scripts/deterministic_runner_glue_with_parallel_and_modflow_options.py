...
class DeterministicRunner(DynamicModel):
    ...
    def __init__(
        self, configuration, modelTime, initialState=None, system_argument=None
    ):
...
def main():
    ...
    if len(sys.argv) > 2:
        if sys.argv[2] == "parallel" or sys.argv[2] == "debug_parallel":
    ...
    if noSpinUps > 0:
        ...
        while spinUpRun < noSpinUps and has_converged == False:
            ...
            deterministic_runner = DeterministicRunner(
                configuration, currTimeStep, initial_state, sys.argv
            )
