    ...
    def report_summary(
        ...
        if self._modelTime.doy == 1 or self._modelTime.isFirstTimestep():
            ...
            for var in self.landSurface.fluxVars:
                vars(self)[var + "Acc"] = pcr.ifthen(self.landmask, pcr.scalar(0.0))
        ...
        if self._modelTime.isLastDayOfYear() or self._modelTime.isLastTimeStep():
            ...
            variableList += [
                "waterBalance",
                "absWaterBalance",
                "irrigationEvaporationWaterUse",
                "nonIrrigationWaterUse",
            ]

            # consumptive water use for irrigation (unit: m)
            self.irrigationEvaporationWaterUseAcc = (
                vos.getValDivZero(
                    self.irrGrossDemandAcc,
                    self.precipitationAcc + self.irrGrossDemandAcc,
                )
                * self.actualETAcc
            )
